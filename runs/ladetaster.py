...
try:
    while True:
        ...
        if button2_state == False:
            file = open("/var/www/html/openWB/ramdisk/nurpv70dynstatus","w")
            file.write("1")
            file.close()
            file = open("/var/www/html/openWB/ramdisk/lademodus","w")
            file.write("2")
            file.close()
            file = open("/var/www/html/openWB/ramdisk/ladestatus.log","a") 
            file.write("Lademodus geaendert durch Ladetaster auf PV 70%")
            file.close()
            time.sleep(0.2)
        if button3_state == False:
            ...
            file.close()
            file = open("/var/www/html/openWB/ramdisk/ladestatus.log","a") 
            file.write("Lademodus geaendert durch Ladetaster auf nur PV")
            file.close()
            time.sleep(0.2)
